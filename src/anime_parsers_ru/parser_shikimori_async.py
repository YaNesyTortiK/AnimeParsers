...
class ShikimoriParserAsync:
    ...
    async def search(self, title: str) -> list:
        """
        ...
        response = await self.requests.get(f'https://{self._dmn}/animes/autocomplete/v2', params=params, headers=headers)
    ...
    async def anime_info(self, shikimori_link: str) -> dict:
        """
        ...
        response = await self.requests.get(shikimori_link, headers=headers)
        if response.status_code == 429:
            raise errors.TooManyRequests(f'Сервер вернул код 429 для обозначения что запросы выполняются слишком часто.')
        elif response.status_code == 520:
            raise errors.ServiceIsOverloaded("Сервер вернул статус ответа 520, что означает что он перегружен и не может ответить сразу.")
    ...
    async def additional_anime_info(self, shikimori_link: str) -> dict:
        """
        ...
        response = await self.requests.get(link, headers=headers)
        if response.status_code == 429:
            raise errors.TooManyRequests(f'Сервер вернул код 429 для обозначения что запросы выполняются слишком часто.')
        elif response.status_code == 520:
            raise errors.ServiceIsOverloaded("Сервер вернул статус ответа 520, что означает что он перегружен и не может ответить сразу.")
    ...
    async def get_anime_list(self, status: list[str] = [], anime_type: list[str] = [], rating: str | None = None, genres: list[str] = [], start_page: int = 1, page_limit: int = 3, sort_by: str = 'rating') -> list:
        ...
        while i < start_page+page_limit and i <= total_pages:
            response = await self.requests.get(f'{search_url}/page/{i}.json?order={sort_by}{f"&rating={rating}" if rating != None else ""}', headers=headers)
            if response.status_code == 429:
                raise errors.TooManyRequests(f'Сервер вернул код 429 для обозначения что запросы выполняются слишком часто.')
            elif response.status_code == 520:
                raise errors.ServiceIsOverloaded("Сервер вернул статус ответа 520, что означает что он перегружен и не может ответить сразу.")
    ...
    def _get_anime_info_from_article(self, article: Soup) -> dict:
        ...
        try:
            ...
            pass
        return c_data
    async def get_ongoing_calendar(self) -> dict[str: list[dict]]:
        """
...
...
        ...
        response = await self.requests.get(f'https://{self._dmn}/ongoings', headers=headers)
        response = response.text
        soup = Soup(response, 'lxml') if self.USE_LXML else Soup(response, 'html.parser')
        res = {}
    ...
    async def link_by_id(self, shikimori_id: str) -> str:
        ...
        response = await self.requests.get(f'https://{self._dmn}/animes/{shikimori_id}', headers=headers)
    ...
    async def deep_search(self, title: str, search_parameters: dict = {}, return_parameters: list = ['id', 'name', 'russian', 'genres { id name russian kind}', 'status', 'url']) -> list:
        """
        ...
        response = await self.requests.post(f'https://{self._dmn}/api/graphql', headers=headers, json=json_data)
        if response.status_code == 429:
            raise errors.TooManyRequests(f'Сервер вернул код 429 для обозначения что запросы выполняются слишком часто.')
        elif response.status_code == 520:
            raise errors.ServiceIsOverloaded("Сервер вернул статус ответа 520, что означает что он перегружен и не может ответить сразу.")
    ...
    async def deep_anime_info(self, shikimori_id: str, return_parameters: list = ['id', 'name', 'russian', 'genres { id name russian kind}', 'status', 'url']) -> dict:
        """
        Использует playground для graphql. Эмуляция api без необходимости использовать ключ и без ограничений на количество запросов (предположительно).
        ## ! Возможна блокировка ip, используйте на свой страх и риск !
