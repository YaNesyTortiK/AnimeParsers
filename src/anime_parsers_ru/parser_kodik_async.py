...
class KodikParserAsync:
    ...
    async def search(self, title: str, limit: int|None = None, include_material_data: bool = True, anime_status: str|None = None, strict: bool = False, only_anime: bool = False) -> list:
        """
        ### Для использования требуется токен kodik
        ...
        """
        if limit is None:
            search_data = await self.base_search(title, include_material_data=include_material_data, anime_status=anime_status, strict=strict)
        else:
            search_data = await self.base_search(title, limit, include_material_data=include_material_data, anime_status=anime_status, strict=strict)
    ...
    async def get_embed_link(self, id: str, id_type: str, https: bool = True) -> str:
        """
        ### Для использования требуется токен kodik
        ...
        :id: id медиа
        :id_type: тип id (возможные: shikimori, kinopoisk, imdb, mdl, kodik, worldart_animation, worldart_cinema) (Для кодика нужно указывать в формате: serial-1234 / movie-1234)
        :https: устанавливает будет ли в полученной ссылке https:// или http:// (По умолчанию True)
    ...
    async def get_info(self, id: str, id_type: str) -> dict:
        ...
        link = await self.get_embed_link(id, id_type)
        return await self.get_info_from_embed(link)
    
    async def get_info_from_embed(self, embed_link: str) -> dict:
        """
        ### Для использования требуется токен kodik
        ...
        """
        data = await self.requests.get(embed_link)
        try:
            data = data.text
        except Exception as ex:
            if data.status_code != 200:
    ...
    async def get_link(self, id: str, id_type: str, seria_num: int, translation_id: str) -> tuple[str, int, list[list[int]]]:
        ...
        А также массив таймкодов с опенингами и эндингами, если они есть в виде массива массивов.
        Пример: `[[0, 90], [1150, 1300]]`. Где каждый массив это заставка (обычно первый - опенинг, второй - эндинг, но может быть и только один фрагмент), первое число в массиве начало фрагмента в секундах от начала файла, второе число - конец фрагмента в секундах с начала файла.
